...
def _legacy_build_chapter_asset(markdown: str) -> dict[str, Any]:
    ...
    for galaxy in sorted(galaxy_rows, key=lambda row: _integer(row.get("learning_order"), 9999)):
        ...
        links = [
            {
                "sourceId": edge.get("source_id", "").strip(),
                "targetId": edge.get("target_id", "").strip(),
                "edgeType": edge.get("edge_type", "").strip(),
            }
            for edge in edge_rows
            if edge.get("edge_type", "").strip() == "supports"
            and edge.get("source_id", "").strip() in planet_ids
            and edge.get("target_id", "").strip() in planet_ids
        ]
